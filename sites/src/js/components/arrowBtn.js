...
export function arrowButton() {
    const arrowWrapper = document.querySelectorAll(".intro__bg-btn__wrapper");
    const arrowContainer = document.querySelectorAll(".intro__bg-btn__container");
    const arrowBtn = document.querySelectorAll(".intro__bg-btn");
      ...
      arrowWrapper.forEach((el, i) => {
        arrowWrapper[i].addEventListener("mousemove", (e) => {
          ...
          gsap.to(arrowBtn[i], {
            ...
            x: Math.min(Math.max(x, 80), 90) - arrowBtn[i].offsetWidth / 2,
            duration: 0.5,
          });
        });
